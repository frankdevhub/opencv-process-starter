 ...
 * 
 * @CreateDate:2020年8月4日 下午5:41:08</br>
 * @Version: 1.0</br>
 * @Type:ImageUI.java</br>
...
public class ImageUI extends JComponent {
	...
	public void imshow(String title, Mat src) {
		...
		JDialog ui = new JDialog();// 创建显示的组件对象
		ui.setTitle(title);
		ui.getContentPane().setLayout(new BorderLayout());// 设置布局
		ui.getContentPane().add(this, BorderLayout.CENTER);// 设置布局居中方式
		ui.setSize(image.getWidth() + 16, image.getHeight() + 38);// 设置宽高

		ui.setVisible(true);// 设置布局居中方式
		this.repaint();
	}
		...
		for (int row = 0; row < height; row++) {
			for (int col = 0; col < width; col++) {
				...
				if (dims == 3) {
					...
					pixels[row * width + col] = ((255 & 0xff) << 24) | ((r & 0xff) << 16) | ((g & 0xff) << 8)
							| (b & 0xff);
				} else if (dims == 1) {
					b = data[index] & 0xff;
					pixels[row * width + col] = ((255 & 0xff) << 24) | ((b & 0xff) << 16) | ((b & 0xff) << 8)
							| (b & 0xff);
				}
			}
		}
